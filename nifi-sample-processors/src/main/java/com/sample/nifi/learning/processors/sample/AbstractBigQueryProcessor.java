...
/**
 * AbstractBigqueryProcessor is a base class for Bigquery processors and
 * contains logic and variables common to most processors integrating with
 * Google Bigquery.
 */
public abstract class AbstractBigQueryProcessor extends AbstractProcessor {
	...
	static final PropertyDescriptor SERVICE_ACCOUNT_CREDENTIALS_JSON = new PropertyDescriptor.Builder()
	    ...
	    .description(
	        "Service Account Credentials permit to authenticate an application to the Google Services. The credentials should be "
	            + "provided to connect on bigquery service")
	    .required(true).addValidator(StandardValidators.NON_EMPTY_VALIDATOR).build();

	static final PropertyDescriptor CONNECTION_TIMEOUT = new PropertyDescriptor.Builder().name("Connect Timeout")
	    .description("Timeout to set a connection with BigQuery").required(false)
	    .addValidator(StandardValidators.INTEGER_VALIDATOR).build();

	static final PropertyDescriptor READ_TIMEOUT = new PropertyDescriptor.Builder().name("Read Timeout")
	    .description("Timeout to read data from an established connection with Bigquery Service").required(false)
	    .addValidator(StandardValidators.INTEGER_VALIDATOR).build();

	static final PropertyDescriptor PROJECT = new PropertyDescriptor.Builder().name("Google Cloud Project").description(
	    "The project id where find dataset and table. If not provided will be used the project specified on service account credentials")
	    .required(false).addValidator(StandardValidators.StringLengthValidator.VALID).build();
	...
	protected BigQuery createBigquery(ProcessContext context) throws BigQueryInitializationException {
		...
		try (ByteArrayInputStream serviceAccountCredentialsJsonByteArray = new ByteArrayInputStream(
		    serviceAccountCredentialsJsonProperty.getValue().getBytes(StandardCharsets.UTF_8))) {
			bigQueryOptionBuilder
			    .setCredentials(ServiceAccountCredentials.fromStream(serviceAccountCredentialsJsonByteArray));
		} catch (IOException e) {
		...
		if (projectIdProperty.isSet()) {
			...
			try (ByteArrayInputStream serviceAccountCredentialsJsonByteArray = new ByteArrayInputStream(
				...
				throw new BigQueryInitializationException(
				    "A project id is required but could not be determined from the properties or Service Account Credentials",
				    e);
			}
		}
	 ...
	 *
	 * @param serviceAccountCredentialsJson Json Stream of Service Account
	 *                                      Credentials Stream
